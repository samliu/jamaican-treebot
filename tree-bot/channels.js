...
//For sam's prank server
function sam_speak(msg){
  //Post the speech...
  var util = require('util'),
      exec = require('child_process').exec,
      child;

  child = exec('python post.py '+msg,
...
var interceptors = [
  { 'chance' : 0.10, 'fn' : welshify },
  { 'chance' : 0.30, 'fn' : mehdify },
];
...
ChannelController.prototype = {
  ...
  currently: function(from){
    ...
    sam_speak("Let me show you what trees im watching");
    ...
    for (var x in watches){
      ...
      sam_speak("Watching revision "+watches[x].rev+" for "+watches[x].name+" on tree "+watches[x].tree);
      count++;
    }
    if(count == 0){
